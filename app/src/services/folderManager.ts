...
export const parseFolderLine = (line: string): { uid: string; name: string } | null => {
  if (!line || !line.includes('_File')) return null;
  const raw = line.split('_File')[0]; // e.g., "123456 - Documents"
  if (raw.includes(' - ')) {
...
export const fetchAllIndexMessages = async (client: TelegramClient, entityStr: string) => {
  let allMatches: any[] = [];
  let offsetId = 0;
  while (true) {
...
export const deleteFolderFiles = async (
  ...
  for (let i = 0; i < ids.length; i += 100) {
    ...
    onProgress?.(`Deleting files ${i + 1} to ${Math.min(i + 100, ids.length)} of ${ids.length}...`);
